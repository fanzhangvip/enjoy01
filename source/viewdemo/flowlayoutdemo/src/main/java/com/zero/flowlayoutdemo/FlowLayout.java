...
public class FlowLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		Log.i(TAG, "onMeasure: sizeHeight= " + sizeHeight);

        int modeWidth = MeasureSpec.getMode(widthMeasureSpec);
        int modeHeight = MeasureSpec.getMode(heightMeasureSpec);
		Log.i(TAG, "onMeasure: modeWidth= " + getModeString(modeWidth));
		Log.i(TAG, "onMeasure: modeHeight= " + getModeString(modeHeight));
        ...
        for(int i = 0; i < childCount; i++) {
			...
			Log.i(TAG, "onMeasure: child= " + child + " lastChild= " + lastChild);

            if(child.getVisibility() == View.GONE) {//处理不显示的View
